import GraphMatrix.Main.NoActiveLower
...
theorem main_zeroRole_corePowerScale_eq_one
    (P : PaperShape) (hroles : P.roles = 0) (n : ℕ) :
    mainCorePowerScale P n = 1 := by
...
theorem main_zeroResidual_originalLower
    (G : PaperShape)
    (hzero : (mainIsolatedReducedShape G).roles = 0) :
    ∃ c : ℝ, 0 < c ∧ ∃ N : ℕ, ∀ n : ℕ, N ≤ n →
      c * mainOriginalPowerScale G n ≤
        paperMean (fun w : PaperNoise n => ‖paperGraphMatrix G n w‖) := by
  have hReduced : ∀ n : ℕ, 1 ≤ n →
      (1 : ℝ) * mainCorePowerScale (mainIsolatedReducedShape G) n ≤
          paperMean (fun w : PaperNoise n =>
            ‖paperGraphMatrix (mainIsolatedReducedShape G) n w‖) ∧
        paperMean (fun w : PaperNoise n =>
            ‖paperGraphMatrix (mainIsolatedReducedShape G) n w‖) ≤
          (1 : ℝ) * mainCorePowerScale (mainIsolatedReducedShape G) n := by
    intro n hn
    have hScale := main_zeroRole_corePowerScale_eq_one
      (mainIsolatedReducedShape G) hzero n
    have hMean : paperMean (fun w : PaperNoise n =>
        ‖paperGraphMatrix (mainIsolatedReducedShape G) n w‖) = 1 := by
      simp only [paperR16_zeroRole_graphMatrix_norm_eq_one
        (mainIsolatedReducedShape G) hzero, paperMean_const_function]
    rw [hScale, hMean]
    norm_num
  have h := main_isolated_sharp_twoSided_transfer G 1 1
...
theorem main_noActiveMaximum_originalLower_all
    (G : PaperShape)
    (hZero : G.toPartiteShape.activeComponentMaximum = 0) :
    ∃ c : ℝ, 0 < c ∧ ∃ N : ℕ, ∀ n : ℕ, N ≤ n →
      c * mainOriginalPowerScale G n ≤
        paperMean (fun w : PaperNoise n => ‖paperGraphMatrix G n w‖) := by
  by_cases hroles : (mainIsolatedReducedShape G).roles = 0
  · exact main_zeroResidual_originalLower G hroles
  · exact main_noActiveMaximum_originalLower G (Nat.pos_of_ne_zero hroles) hZero
