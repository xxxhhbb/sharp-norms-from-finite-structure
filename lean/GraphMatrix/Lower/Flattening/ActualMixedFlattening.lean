...
/-!
# semantic reindexing of the actual mixed flattening

The key convention is fixed once and for all here:
`freshDir g = !(edgeSide ... e)`.  Thus row edges (`edgeSide=false`)
...
/-! ## 4. Payloads are exactly fresh-edge address fields -/
...
/-- A row edge is exactly a fresh group with `freshDir=true`. -/
...
/-- A column edge is exactly a fresh group with `freshDir=false`. -/
...
the NEW mixed index.  This is the orientation used in the displayed identity. -/
def actualRowEquiv :
    RowAddress P S (X dimension) ≃
      lowerMixedRows (freshCount P S) (freshSize P S dimension) (freshDir P S)
        (PartiteBoundaryRow (G := P.toPartiteShape) dimension) :=
...
/-- the actual mixed flattening is entrywise the retained-address
...
/-- The reindexing norm identity follows from the
