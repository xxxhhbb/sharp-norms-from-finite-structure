...
import GraphMatrix.Model.EventuallyTypedCoreLp
...
def mainTraceScale (C n : ℝ) (k a b s : ℕ) : ℝ :=
  (2 * C ^ (2 * k) * (k : ℝ) ^ (a * k) * n ^ (k * b + s)) ^
    ((2 * (k : ℝ))⁻¹)

theorem mainTraceScale_eq_factors (C n : ℝ) (k a b s : ℕ)
    (hC : 0 < C) (hn : 0 < n) (hk : 0 < k) :
    mainTraceScale C n k a b s =
      (2 : ℝ) ^ ((2 * (k : ℝ))⁻¹) * C *
        (k : ℝ) ^ ((a : ℝ) / 2) * n ^ ((b : ℝ) / 2) *
          n ^ ((s : ℝ) / (2 * (k : ℝ))) := by
  have hkR : (0 : ℝ) < k := by exact_mod_cast hk
  have hkNe : (k : ℝ) ≠ 0 := ne_of_gt hkR
  unfold mainTraceScale
...
theorem main_separator_rpow_le_exp (n k s : ℝ)
...
theorem mainTraceScale_le_log_scale (C n D : ℝ) (k a b s : ℕ)
  ...
  have hsep := main_separator_rpow_le_exp n k s hnPos (by positivity) (by positivity) hcover
  rw [mainTraceScale_eq_factors C n k a b s hC hnPos hkPos]
  gcongr

theorem main_paper_finiteScale_eq_traceScale
    (G : PartiteShape) (p s a n : ℕ) :
    paperR16TypedCoreFiniteLpScale G p s a n =
      mainTraceScale (c079C G.roles) n (p + 1) a (G.roles - s) s := by
  unfold paperR16TypedCoreFiniteLpScale mainTraceScale c079BlockTarget
...
theorem main_paper_finiteScale_le_log_scale
  ...
  rw [main_paper_finiteScale_eq_traceScale,
    paperR16ReplicaParameter_add_one]
  apply mainTraceScale_le_log_scale _ _ _ _ _ _ _ hC hnR
...
theorem main_paper_finiteScale_le_original_log_scale
    (G : PartiteShape) (s a n : ℕ) (q C₀ : ℝ)
  ...
  calc
    paperR16TypedCoreFiniteLpScale G (paperR16ReplicaParameter n q) s a n ≤
        2 * (c079C G.roles : ℝ) *
          ...
          (n : ℝ) ^ ((G.roles - s : ℕ) / (2 : ℝ)) *
          Real.exp ((s : ℝ) / 2) :=
      main_paper_finiteScale_le_log_scale G s a n q C₀ hn hC₀ hq hqWindow
    _ ≤ 2 * (c079C G.roles : ℝ) *
