...
theorem main_mean_norm_le_finiteUniformL2
...
def mainBoundaryCoreUpperConstant (G : PaperShape) : ℝ :=
  (G.roles : ℝ) ^ G.roles *
    (2 * (c079C G.roles : ℝ) *
      Real.exp ((G.toPartiteShape.rightLeftSeparatorNumber : ℝ) / 2) *
      (8 : ℝ) ^ ((G.toPartiteShape.activeComponentMaximum : ℝ) / 2))

theorem mainBoundaryCoreUpperConstant_pos (G : PaperShape) (hr : 0 < G.roles) :
    0 < mainBoundaryCoreUpperConstant G := by
  have hC : 0 < (c079C G.roles : ℝ) := by
    unfold c079C
    positivity
  unfold mainBoundaryCoreUpperConstant
  positivity
...
theorem main_boundaryCore_globalMean_sharp_of_exactCount
    (hCount : ReplicaCounting.ExactStratumBound)
    (G : PaperShape) (hr : 0 < G.roles) (hCore : G.toPartiteShape.IsBoundaryCore) :
    ∃ N : ℕ, ∀ n : ℕ, N ≤ n →
      paperMean (fun w : PaperNoise n => ‖paperGraphMatrix G n w‖) ≤
        mainBoundaryCoreUpperConstant G *
          (n : ℝ) ^ (((G.roles : ℝ) -
  ...
  have hTyped : ∀ dimension : Fin G.roles → ℕ, (∀ v, dimension v ≤ n) →
    ...
    have hScale : paperR16TypedCoreFiniteLpScale H (paperR16ReplicaParameter n 2) s a n ≤ T := by
      simpa only [T, H, s, a, G.toPartiteShape_roles,
        ...
        main_paper_finiteScale_le_original_log_scale
  ...
  calc
    ...
    _ = _ := by dsimp [T, mainBoundaryCoreUpperConstant, s, a, H]; ring

/-- Unconditional boundary-core upper bound for the actual globally-injective
paper matrix.  The U3 exact count is now supplied by its proved coefficient
      ...
      paperMean (fun w : PaperNoise n => ‖paperGraphMatrix G n w‖) ≤
        mainBoundaryCoreUpperConstant G *
          (n : ℝ) ^ (((G.roles : ℝ) -
            (G.toPartiteShape.rightLeftSeparatorNumber : ℝ)) / 2) *
          Real.log (n : ℝ) ^
            ((G.toPartiteShape.activeComponentMaximum : ℝ) / 2) :=
  main_boundaryCore_globalMean_sharp_of_exactCount
    ReplicaCounting.exactStratumBound G hr hCore
