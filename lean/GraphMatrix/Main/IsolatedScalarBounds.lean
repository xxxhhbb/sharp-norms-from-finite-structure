...
theorem main_isolated_scalar_le_pow (n v h : ℕ) :
...
theorem main_pow_le_scaled_isolated_scalar (n v h : ℕ)
    (hh : h ≤ v) (hn : 2 * v ≤ n) :
...
theorem main_isolated_scalar_real_bounds (n v h : ℕ)
    (hh : h ≤ v) (hn : 2 * v ≤ n) :
  ...
  · have hscaled : (n : ℝ) ^ h ≤
      ...
      exact_mod_cast main_pow_le_scaled_isolated_scalar n v h hh hn
    rw [div_pow]
    apply (div_le_iff₀ (by positivity : (0 : ℝ) < 2 ^ h)).2
    simpa only [mul_comm] using hscaled
  · exact_mod_cast main_isolated_scalar_le_pow n v h
