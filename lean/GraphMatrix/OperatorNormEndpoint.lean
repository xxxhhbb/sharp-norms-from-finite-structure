...
This module is the downstream endpoint for a trace-to-norm-style trace budget.
...
/-- The finite-uniform `L^q` norm used by the core upper endpoint.
...
/-- Abstract trace-to-norm transfer.  A dyadic Gram-trace estimate at order `p=2^q`
