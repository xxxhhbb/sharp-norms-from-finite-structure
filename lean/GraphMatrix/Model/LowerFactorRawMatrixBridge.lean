import GraphMatrix.Model.LowerFactorComponentReindex
...
theorem rawEntry_eq_preprocessed_components
  ...
  simp only [GraphMatrixReplica.Model.PreprocessedFactorShape.coreEntry]
...
theorem rawMatrix_eq_canonicalFullMatrix (ξ : S.RawSample)
  ...
  classical
  unfold rawMatrix GraphMatrixReplica.Model.PreprocessedFactorShape.fullMatrix
  calc
    (∑ x : S.RawAssignment,
      if (∀ w : {w : W // w ∈ S.leftBoundary}, x w.1 = row w) ∧
          (∀ w : {w : W // w ∈ S.rightBoundary}, x w.1 = col w) then
