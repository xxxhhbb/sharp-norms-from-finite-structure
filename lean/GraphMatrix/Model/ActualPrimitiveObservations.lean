import GraphMatrix.Model.ComponentSide
import GraphMatrix.Model.MinimumSeparatorPath
import GraphMatrix.ToPartiteBridge

set_option autoImplicit false

/-!
# actual primitive observations after deleting active components
...
`cut` by definition. The abstract `Separator.rowRoles/colRoles` are
...
namespace GraphMatrixReplica.Model.ActualPrimitiveObservations

open GraphMatrixReplica.Model.Separator
...
theorem rowObserved_subset_abstract_rowRoles
    (hSep : G.IsRightLeftSeparator cut) :
    rowObserved G cut ⊆ Separator.rowRoles G cut := by
  intro v hv
  rcases (mem_rowObserved_iff G cut v).1 hv with hLeft | ⟨e, _, hSide, hve⟩
  · exact Separator.leftBoundary_subset_rowRoles G cut hSep hLeft
...
theorem colObserved_subset_abstract_colRoles :
    colObserved G cut ⊆ Separator.colRoles G cut := by
  intro v hv
  rcases (mem_colObserved_iff G cut v).1 hv with hRight | ⟨e, _, hSide, hve⟩
  · exact Separator.rightBoundary_subset_colRoles G cut hRight
  · by_cases hvCut : v ∈ cut
...
theorem observed_inter_subset_cut (hSep : G.IsRightLeftSeparator cut) :
  ...
  have hBoth : v ∈ Separator.rowRoles G cut ∩
      Separator.colRoles G cut := Finset.mem_inter.2 ⟨hR, hC⟩
...
/-- The nontrivial obligation: every separator role is observed by actual
...
end GraphMatrixReplica.Model.ActualPrimitiveObservations
