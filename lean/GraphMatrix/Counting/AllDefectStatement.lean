...
/-! # The C079 statement at every natural-number defect

The earlier coefficient interface uses `Fin (target + 1)` because those are
exactly the potentially nonempty fibers.  states its theorem for every
...
/-- The degenerate role-zero case has exactly one admissible state. -/
