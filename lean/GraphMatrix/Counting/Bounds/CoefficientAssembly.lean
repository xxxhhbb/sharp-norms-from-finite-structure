import GraphMatrix.Counting.Bounds.PathFiberRestriction
import GraphMatrix.Counting.Bounds.SeedEntropyCharge
import GraphMatrix.Counting.Bounds.MengerProfileTransport
...
theorem main_pathFiber_card_le_requested
...
theorem main_defectFiber_card_le_fixedProfileBudget
  ...
  have hPath : Fintype.card (PathFiber p family d) ≤ pathBound :=
    main_pathFiber_card_le_requested hp family d S
  have hSeed : ∀ (B : PathFiber p family d)
    ...
    intro B forward
    exact main_seedFiber_card_le_requested hCore hOpt B forward
...
theorem main_defectFiber_card_le_fixedProfileBudget_all
    (hp : 1 ≤ p) (hr : 1 ≤ G.roles) (hCore : G.IsBoundaryCore)
  ...
  · obtain ⟨S⟩ := h
    exact main_defectFiber_card_le_fixedProfileBudget hp hr hCore menger d S
...
theorem main_sum_defectFiber_cards_eq_sum_realizable
...
certificate. -/
theorem main_actual_coefficientNat_le_requested
  ...
  rw [main_sum_defectFiber_cards_eq_sum_realizable]
  calc
    (∑ d : RealizableProfile G p G.rightLeftPathPackingNumber delta,
      ...
      intro d _
      exact main_defectFiber_card_le_fixedProfileBudget hp hr hCore menger d.1.1
    ...
    _ ≤ c079C G.roles ^ (2 * (p + 1)) *
      ...
      sum_realizable_profile_budgets_le_requested G p
        G.rightLeftPathPackingNumber delta G.activeComponentMaximum
        (G.roleCovered_of_isBoundaryCore hCore) family hr

/-- Finite-indexed form used by the existing wrapper. -/
theorem main_actual_coefficient_le_requested
    (G : PartiteShape) (p : Nat) (hp : 1 ≤ p) (hr : 1 ≤ G.roles)
    (hCore : G.IsBoundaryCore) (menger : G.RightLeftMengerCertificate)
  ...
  main_actual_coefficientNat_le_requested G p hp hr hCore menger delta.1
