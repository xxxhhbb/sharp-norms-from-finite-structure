import GraphMatrix.Counting.Bounds.RecordSemantics
import GraphMatrix.Counting.CompatibleMatchingSeedCount
import GraphMatrix.Counting.MatchingSwitchDescentCore
import GraphMatrix.Counting.PartitionInterval
...
theorem main_matching_refines_of_edgeParity {m : ℕ}
...
theorem main_even_of_matching_refinement {m : ℕ}
    (π : ReplicaPartition m) (σ : C079MatchingPartition m)
...
theorem main_edgeParity_symm {m : ℕ} {π σ : ReplicaPartition m}
...
theorem main_state_edgeParity_incident {G : PartiteShape} {p : ℕ}
  ...
  · exact main_edgeParity_symm (T.edgeParity e)
  · exact (hxy rfl).elim
...
theorem main_seed_refines_neighbor (seed : SeedFiber B forward)
  ...
  have hxy : x ≠ y := by
    ...
    subst y
    exact hyOff hxBack
  have hpar := main_state_edgeParity_incident T e x y hxy hex hey
  ...
  rw [hxT, hyT] at hpar
  exact main_matching_refines_of_edgeParity _ _ hpar

/-- Actual validity supplies the refinement; no compatibility assumption is added. -/
theorem main_seed_refines_neighborMeet (seed : SeedFiber B forward)
    (c : G.C079CutComponent family.backboneRoles) :
  ...
  apply Finset.le_inf
  intro x hx
  exact main_seed_refines_neighbor seed c x hx

theorem main_neighborMeet_even_of_seed (seed : SeedFiber B forward)
    (c : G.C079CutComponent family.backboneRoles) :
    IsEvenPartition (neighborMeet B forward c) :=
  main_even_of_matching_refinement _ _ (main_seed_refines_neighborMeet seed c)

/-- also bounds an empty compatible-seed type, without an external evenness premise. -/
theorem main_compatibleSeed_card_le {m : ℕ} (θ : ReplicaPartition m) :
  ...
  · obtain ⟨σ⟩ := h
    exact c079_compatibleSeed_card_le θ
      (main_even_of_matching_refinement θ σ.1 σ.2)
...
theorem main_seedFiber_card_le_product
    (B : PathFiber p family d)
    (forward : ForwardCode (Fin G.roles) (p + 1) (3 * G.roles * offDefect family d)) :
  ...
  let f : SeedFiber B forward →
    ...
    fun seed c => ⟨seed.1 c.1, main_seed_refines_neighborMeet seed c.1⟩
  ...
  calc
    ...
    _ ≤ _ := by
      ...
      simpa only [Nat.card_eq_fintype_card] using
        main_compatibleSeed_card_le (neighborMeet B forward c.1)

def mainSeedEntropy (B : PathFiber p family d)
    (forward : ForwardCode (Fin G.roles) (p + 1) (3 * G.roles * offDefect family d)) : ℕ :=
  ∑ c : FreeComponent family,
    ((p + 1) - partitionBlockCount (neighborMeet B forward c.1))

theorem main_freeComponent_card_le_roles
...
theorem main_seedEntropy_le_roles_mul_order
    (B : PathFiber p family d)
    (forward : ForwardCode (Fin G.roles) (p + 1) (3 * G.roles * offDefect family d)) :
    mainSeedEntropy B forward ≤ G.roles * p := by
  classical
  have hEach (c : FreeComponent family) :
    ...
    omega
  calc
    mainSeedEntropy B forward ≤ ∑ _c : FreeComponent family, p :=
      Finset.sum_le_sum (fun c _ => hEach c)
    _ = Fintype.card (FreeComponent family) * p := by simp
    _ ≤ G.roles * p := Nat.mul_le_mul_right p (main_freeComponent_card_le_roles family)

theorem main_seedFiber_card_le_entropy
    (B : PathFiber p family d)
    (forward : ForwardCode (Fin G.roles) (p + 1) (3 * G.roles * offDefect family d)) :
    Nat.card (SeedFiber B forward) ≤ (2 * (p + 1)) ^ mainSeedEntropy B forward := by
  have h := main_seedFiber_card_le_product B forward
