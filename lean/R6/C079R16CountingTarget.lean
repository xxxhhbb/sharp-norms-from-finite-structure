import R6.C079AllDefectStatement
...
/-- R16 Theorem `Uniform all-defect count`, for positive role count and every
...
theorem c079_r16_count_of_exactStratumCertificate
    ...
    (hCount : ∀ d : Fin
      ...
      c079DefectCoefficient G p G.rightLeftSeparatorNumber d ≤
        c079C G.roles ^ (2 * (p + 1)) *
          (p + 1) ^
            (G.c079ActiveMaximum * (p + 1) + c079K G.roles * d.1)) :
    ∀ delta : ℕ,
  ...
  exact c079_allDefect_bound_of_exactFiberCount
    G p G.rightLeftSeparatorNumber G.c079ActiveMaximum
      (c079K G.roles) (c079C G.roles) hCount

#print axioms PartiteShape.mengerCut_card_eq_separatorNumber
#print axioms c079_r16_count_of_exactStratumCertificate
