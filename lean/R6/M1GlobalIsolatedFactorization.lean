...
theorem root_left_mem_covered (G : PaperShape) (i : Fin G.leftSize) :
...
theorem root_right_mem_covered (G : PaperShape) (i : Fin G.rightSize) :
...
def rootCoveredEntryCompatible (G : PaperShape) {n : ℕ}
    (covered : PaperVisibleTuple n (coveredRoles G.isolatedMiddleRoles))
    (row : PaperRow G n) (col : PaperCol G n) : Prop :=
  (∀ i, covered ⟨G.left i, root_left_mem_covered G i⟩ = row i) ∧
    (∀ j, covered ⟨G.right j, root_right_mem_covered G j⟩ = col j)
...
def rootCoveredGlobalMatrix (G : PaperShape) (n : ℕ) (w : PaperNoise n) :
  ...
  exact fun row col => ∑ covered : PaperVisibleTuple n (coveredRoles G.isolatedMiddleRoles),
    if Function.Injective covered then
      if rootCoveredEntryCompatible G covered row col then paperCoveredNoiseProduct G w covered else 0
    else 0

theorem root_merge_entry_iff (G : PaperShape) {n : ℕ}
    (covered : PaperVisibleTuple n (coveredRoles G.isolatedMiddleRoles))
    ...
    ((∀ i, mergeCoveredIsolatedAssignment G.isolatedMiddleRoles covered hidden (G.left i) = row i) ∧
      ...
      rootCoveredEntryCompatible G covered row col := by
  simp only [rootCoveredEntryCompatible,
    mergeCoveredIsolatedAssignment_apply_covered _ _ _ _ (root_left_mem_covered G _),
    mergeCoveredIsolatedAssignment_apply_covered _ _ _ _ (root_right_mem_covered G _)]
...
theorem root_global_entry_eq_isolated_factor (G : PaperShape) (n : ℕ) (w : PaperNoise n)
    (row : PaperRow G n) (col : PaperCol G n) :
    paperGraphMatrix G n w row col =
      ((n - (coveredRoles G.isolatedMiddleRoles).card).descFactorial G.isolatedMiddleRoles.card : ℕ) *
        rootCoveredGlobalMatrix G n w row col := by
  classical
  ...
  have hInner (covered : PaperVisibleTuple n (coveredRoles G.isolatedMiddleRoles)) :
    ...
    dsimp only [term]
    simp_rw [root_merge_entry_iff, paperAssignmentNoiseProduct_merge_eq]
...
theorem root_global_matrix_eq_isolated_smul (G : PaperShape) (n : ℕ) (w : PaperNoise n) :
    paperGraphMatrix G n w =
      (((n - (coveredRoles G.isolatedMiddleRoles).card).descFactorial G.isolatedMiddleRoles.card : ℕ) : ℝ) •
        rootCoveredGlobalMatrix G n w := by
  ext row col
  exact root_global_entry_eq_isolated_factor G n w row col

theorem root_global_norm_eq_isolated_mul (G : PaperShape) (n : ℕ) (w : PaperNoise n) :
    ‖paperGraphMatrix G n w‖ =
      ((n - (coveredRoles G.isolatedMiddleRoles).card).descFactorial G.isolatedMiddleRoles.card : ℕ) *
        ‖rootCoveredGlobalMatrix G n w‖ := by
  rw [root_global_matrix_eq_isolated_smul, norm_smul, Real.norm_of_nonneg (by positivity)]

theorem root_global_mean_eq_isolated_mul (G : PaperShape) (n : ℕ) :
    paperMean (fun w : PaperNoise n => ‖paperGraphMatrix G n w‖) =
      ((n - (coveredRoles G.isolatedMiddleRoles).card).descFactorial G.isolatedMiddleRoles.card : ℕ) *
        paperMean (fun w : PaperNoise n => ‖rootCoveredGlobalMatrix G n w‖) := by
  simp_rw [root_global_norm_eq_isolated_mul]
  unfold paperMean
  rw [← Finset.mul_sum]
  ring

#print axioms root_global_entry_eq_isolated_factor
#print axioms root_global_matrix_eq_isolated_smul
#print axioms root_global_norm_eq_isolated_mul
#print axioms root_global_mean_eq_isolated_mul
