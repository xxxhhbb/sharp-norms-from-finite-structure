...
/-! # R16 integer-order norm-to-Gram-trace domination

The core upper bound in R16 uses arbitrary integer trace order, not only the
...
by R16 `prop:core-upper`.  The matrices may be rectangular, and `p ≥ 1` is
...
#print axioms paperR16_matrix_l2_opNorm_pow_le_gramTrace
