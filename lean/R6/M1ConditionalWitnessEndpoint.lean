import R6.M1FrozenWitnessLaw
import R6.M1InternalGoodFloor
import R6.M1ActiveThresholdProduct
...
theorem root_joint_probability_lower_from_good
  ...
  have hGood := root_event_threshold_mul_le_mean Good
...
def rootSimultaneousComponentEvent (P : PaperShape) (cut : Finset (Fin P.roles))
...
theorem root_uniformWitness_probability_of_conditional
    (P : PaperShape) (cut : Finset (Fin P.roles)) (eps q : ℝ)
    (heps : 0 ≤ eps) (n : ℕ) (hn : 1 ≤ n)
    (hq : q ≤ finiteUniformProbability
      (rootInternalGood P cut (fun _ : Fin P.roles => n / P.roles) n))
    (hConditional : ∀ I : P2a.InternalCube (G := P.toPartiteShape) cut
        (fun _ : Fin P.roles => n / P.roles),
      rootInternalGood P cut (fun _ : Fin P.roles => n / P.roles) n I →
      (1 / 2 : ℝ) ≤ finiteUniformProbability
        (rootSimultaneousComponentEvent P cut (fun _ : Fin P.roles => n / P.roles) eps n I)) :
    q / 2 ≤ rootUniformWitnessProbability P cut
      (eps ^ P.toPartiteShape.c079ActiveComponentCount cut) n := by
  let dimension := fun _ : Fin P.roles => n / P.roles
  let t := rootLowerWitnessThreshold P cut
    (eps ^ P.toPartiteShape.c079ActiveComponentCount cut) n
  let E := rootSimultaneousComponentEvent P cut dimension eps n
  ...
  have hJoint := root_joint_probability_lower_from_good
    (rootInternalGood P cut dimension n) E q (1 / 2) (by norm_num) hq hConditional
  have hMono : finiteUniformProbability (fun p : P2a.InternalCube (G := P.toPartiteShape) cut dimension ×
    ...
    refine ⟨s, ?_⟩
    dsimp [t]
    rw [← root_active_threshold_product P cut eps n hn, Finset.abs_prod]
    apply Finset.prod_le_prod
    · intro K _
      positivity
    · intro K _
      exact hs K
  have hLaw := root_frozen_witness_probability_eq_internalRest
    P.toPartiteShape cut dimension t
  calc
    q / 2 = (1 / 2 : ℝ) * q := by ring
    _ ≤ finiteUniformProbability W := hJoint.trans hMono
    _ = rootUniformWitnessProbability P cut
        (eps ^ P.toPartiteShape.c079ActiveComponentCount cut) n := hLaw.symm

/-- The last P3 input can be a uniform conditional simultaneous-success bound;
...
theorem root_uniformWitness_constants_of_conditional
    ...
    ∃ c p : ℝ, 0 < c ∧ 0 < p ∧ ∃ N' : ℕ, ∀ n : ℕ, N' ≤ n →
      p ≤ rootUniformWitnessProbability P cut c n := by
  obtain ⟨q, hq, M, hM, hGood⟩ := root_internalGood_uniform_floor P hr
  refine ⟨eps ^ P.toPartiteShape.c079ActiveComponentCount cut, q / 2,
    pow_pos heps _, by positivity, max N M, ?_⟩
  intro n hn
  exact root_uniformWitness_probability_of_conditional P cut eps q heps.le n
    (hM.trans ((le_max_right _ _).trans hn))
    (hGood cut n ((le_max_right _ _).trans hn))
    (hConditional n ((le_max_left _ _).trans hn))

#print axioms root_joint_probability_lower_from_good
#print axioms root_uniformWitness_probability_of_conditional
#print axioms root_uniformWitness_constants_of_conditional
