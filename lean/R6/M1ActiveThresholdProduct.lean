...
theorem root_active_threshold_product (P : PaperShape) (S : Finset (Fin P.roles))
    (eps : ℝ) (n : ℕ) (hn : 1 ≤ n) :
    (∏ K : P2a.ActiveComponent P.toPartiteShape S,
      eps * (n : ℝ) ^ ((Fintype.card (P2a.ComponentRole K) : ℝ) / 2) *
        Real.sqrt (Real.log (n : ℝ))) =
      rootLowerWitnessThreshold P S (eps ^ P.toPartiteShape.c079ActiveComponentCount S) n := by
  ...
  have hpower :
      ...
      (n : ℝ) ^ ((rootC2ActiveRoleCount P S : ℝ) / 2) := by
    rw [← Real.rpow_sum_of_pos hn0]
    congr 1
    simp [rootC2ActiveRoleCount, Nat.cast_sum, Finset.sum_div]
  have hsqrt :
      (Real.sqrt (Real.log (n : ℝ))) ^
    ...
    rw [Real.sqrt_eq_rpow, ← Real.rpow_natCast,
      ← Real.rpow_mul hlog, root_activeComponent_card_eq_count]
    congr 1
    ring
  simp only [Finset.prod_mul_distrib, Finset.prod_const, Finset.card_univ]
  rw [hpower, hsqrt, root_activeComponent_card_eq_count]
  rfl

/-- Simultaneous lower bounds must use one and the same actual separator tuple.
Their product gives the actual C2 witness threshold without separate maxima. -/
theorem root_actualWeight_lower_of_component_thresholds
    ...
    rootLowerWitnessThreshold P S (eps ^ P.toPartiteShape.c079ActiveComponentCount S) n ≤
      |actualWeight (G := P.toPartiteShape) S dimension ω s| := by
  rw [← root_active_threshold_product P S eps n hn, actualWeight, Finset.abs_prod]
...
theorem root_uniform_witness_of_simultaneous_components
  ...
  obtain ⟨s, hs⟩ := hEvent
  exact ⟨s, root_actualWeight_lower_of_component_thresholds P S _ eps heps n hn ω s hs⟩

#print axioms root_active_threshold_product
#print axioms root_actualWeight_lower_of_component_thresholds
#print axioms root_uniform_witness_of_simultaneous_components
