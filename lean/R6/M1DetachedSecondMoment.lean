...
import R6.X3aScalarMomentProof
...
theorem root_rademacherSign_power_sum (m : ℕ) :
...
theorem paperMean_rademacherSign_monomial
  ...
  dsimp only at heq ⊢
  rw [← heq]
  simp_rw [root_rademacherSign_power_sum]
...
theorem root_map_bool_sign_eq_pi_rootScalarSignLaw
    (K : Type) [Fintype K] [DecidableEq K] :
    Measure.map
        (fun epsilon : K → Bool => fun k =>
          (GraphMatrixReplica.rademacherSign (epsilon k) : ℝ))
        (GraphMatrixReplica.rootFiniteUniformLaw (K → Bool)) =
      Measure.infinitePi (fun _ : K => GraphMatrixReplica.rootScalarSignLaw) := by
  rw [GraphMatrixReplica.root_uniform_bool_function_law]
  exact Measure.infinitePi_map_pi
    (fun _ : K => GraphMatrixReplica.rootFiniteUniformLaw Bool)
...
theorem integral_abs_detachedScalar_sq_rootScalarSignLaw
    ...
    (∫ x, |S.detachedScalarFromBlock j x| ^ 2
      ∂S.oneBlockLaw (fun _ => GraphMatrixReplica.rootScalarSignLaw)
        (Sum.inr j)) =
      Fintype.card (S.canonicalPreprocessedShape.DetachedTuple j) := by
  change
    (∫ x : S.DetachedPrimitiveCoord j → ℝ,
      ...
      ∂Measure.infinitePi
        (fun _ : S.DetachedPrimitiveCoord j =>
          GraphMatrixReplica.rootScalarSignLaw)) = _
  rw [← root_map_bool_sign_eq_pi_rootScalarSignLaw
    (S.DetachedPrimitiveCoord j)]
  ...
  rw [GraphMatrixReplica.root_integral_finiteUniformLaw_eq_paperMean]
...
#print axioms detachedAssignmentSupport_injective
#print axioms paperMean_detachedAmplitude_pair
#print axioms paperMean_detachedScalar_sq
#print axioms integral_abs_detachedScalar_sq_rootScalarSignLaw
#print axioms integral_abs_detachedScalar_sq_eq_roleDimensionProduct
#print axioms integral_abs_detachedScalar_sq_eq_pow_of_constantSize
#print axioms integral_abs_detachedScalar_sq_le_pow_of_size_le
#print axioms prod_integral_abs_detachedScalar_sq_le_pow_sum
