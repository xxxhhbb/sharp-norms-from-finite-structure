...
theorem root_componentContraction_joint_eq
...
theorem root_paperMean_prod_fst {A B : Type} [Fintype A] [Fintype B] [Nonempty B]
...
theorem root_contractionFamily_mean
    (G : PartiteShape) (cut : Finset (Fin G.roles)) (dimension : Fin G.roles → ℕ)
  ...
  calc
    _ = paperMean (fun ε : JointEdgeSignSample (G := G) dimension =>
      ...
      (hsplit.trans (root_paperMean_prod_fst f)).symm
    _ = paperMean (fun ε : JointEdgeSignSample (G := G) dimension =>
      ...
      congr 1
      funext s K
      exact root_componentContraction_joint_eq G cut dimension ε s K
    _ = _ := hrest
...
theorem root_frozen_witness_probability_eq_internalRest
  ...
  have h := root_contractionFamily_mean G cut dimension
    (fun F => if ∃ s, t ≤ |∏ K, F s K| then (1 : ℝ) else 0)
  unfold finiteUniformProbability actualWeight
  convert h using 1 <;> congr!

#print axioms root_componentContraction_joint_eq
#print axioms root_paperMean_prod_fst
#print axioms root_contractionFamily_mean
#print axioms root_frozen_witness_probability_eq_internalRest
