...
/-- P1 and P2a count the same actual component-role subtype. -/
theorem root_P1_componentRoleCount_eq (P : PaperShape) (cut : Finset (Fin P.roles))
...
theorem root_uniformFloor_balanced (P : PaperShape) (hr : 0 < P.roles)
  ...
  intro v hv
  have h := root_uniformRoleDimension_bounds P.roles n hr hn
  refine ⟨h.1, ?_⟩
...
The floor dimensions and the full P1 good event are instantiated, not assumed. -/
theorem root_internalGood_uniform_floor (P : PaperShape) (hr : 0 < P.roles) :
    ∃ q : ℝ, 0 < q ∧ ∃ N : ℕ, 1 ≤ N ∧
      ∀ (cut : Finset (Fin P.roles)) (n : ℕ), N ≤ n →
        q ≤ finiteUniformProbability
          (rootInternalGood P cut (fun _ : Fin P.roles => n / P.roles) n) := by
  ...
  have hab : (1 / (2 * (P.roles : ℝ))) ≤ 1 := by
    ...
    nlinarith
  obtain ⟨p, hp, N, hN, hP⟩ :=
    root_internalGood_uniform_graph P (1 / (2 * (P.roles : ℝ))) 1 ha hab
  ...
  have hcard : Fintype.card (P2a.ActiveComponent P.toPartiteShape cut) ≤ P.roles := by
    rw [root_activeComponent_card_eq_count]
  ...
  calc
    ...
    _ ≤ _ := hP cut _ n ((le_max_left _ _).trans hn)
      (fun K => root_uniformFloor_balanced P hr cut K.1 n ((le_max_right _ _).trans hn))

#print axioms root_P1_componentRoleCount_eq
#print axioms root_uniformFloor_balanced
#print axioms root_internalGood_uniform_floor
