...
/-! # Honest expected-norm interface for the C027 partite reduction

`FULLY_PARTITE_REDUCTION.md` compares the original globally-injective paper
matrix with a fully-partite matrix only after deleting isolated middle roles.
For one fixed nonempty reduced shape, the comparison is uniform over all
sufficiently large ambient sizes and uses constants depending on that shape.

This module states exactly that norm-level handoff.  It does **not** identify
the two random matrices, their trace states, or their trace moments.  The
random-balanced-color averaging and edge-color Fourier projection required by
C027 are not yet formalized, so the analytic comparison is packaged as an
explicit hypothesis supplied to downstream theorems; no axiom is introduced.

The degenerate zero-role convention `M = H = [1]` from the markdown proof is
also not conflated with the nonempty-core branch below.  Here `0 < G.roles` is
an explicit field.  Reattaching deleted isolated middle roles is the separate
C005 step and is outside this interface.
-/
...
#print axioms C027NormTransferHypothesis.expectedNormComparison
#print axioms C027NormTransferHypothesis.exists_shapeUniformConstants
