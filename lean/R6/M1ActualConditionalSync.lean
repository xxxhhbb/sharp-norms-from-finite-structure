import R6.P3ActualWeightedSumSync
import R6.M1ActualSimultaneousEvent
...
theorem root_actual_sync_half_uniform_floor
    ...
    ∃ eps : ℝ, 0 < eps ∧ ∃ N : ℕ, 1 ≤ N ∧
      ...
      ∀ I : P2a.InternalCube (G := P.toPartiteShape) cut
        ...
        (1 / 2 : ℝ) ≤ finiteUniformProbability
          (rootSimultaneousComponentEvent P cut
  ...
  have hw : (1 / 2 : ℝ) ≤ finiteUniformProbability
      (rootWeightedTrialSuccess P cut (fun _ : Fin P.roles => n / P.roles)
        (n / P.roles) (fun _ => le_rfl) eps n I) := by
    convert h using 1
    apply root_finiteProbability_congr_instances _ _ _ _ _ _
    intro R
    rfl
  exact hw.trans (root_weightedTrial_probability_le_actual P cut
...
theorem root_actual_uniformWitness_constants
    (P : PaperShape) (hr : 0 < P.roles) (cut : Finset (Fin P.roles)) :
    ∃ c p : ℝ, 0 < c ∧ 0 < p ∧ ∃ N : ℕ, ∀ n : ℕ, N ≤ n →
      p ≤ rootUniformWitnessProbability P cut c n := by
  obtain ⟨eps, heps, N, hN, hsync⟩ := root_actual_sync_half_uniform_floor P hr
  exact root_uniformWitness_constants_of_conditional P hr cut eps heps N
    (fun n hn I hI => hsync cut n hn I hI)

#print root_actual_sync_half_uniform_floor
#print axioms root_actual_sync_half_uniform_floor
#print axioms root_actual_uniformWitness_constants
