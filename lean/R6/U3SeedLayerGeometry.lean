...
def rootCutInclusion (G : PartiteShape) (P S : Finset (Fin G.roles)) (hSP : S ⊆ P) :
...
def RootComponentIsolatedAt {G : PartiteShape} {P : Finset (Fin G.roles)}
...
theorem root_component_mem_map (G : PartiteShape)
    (P S : Finset (Fin G.roles)) (hSP : S ⊆ P)
    (c : G.C079CutComponent P) (v : Fin G.roles)
    (hv : v ∈ G.c079ComponentRoles P c) :
    v ∈ G.c079ComponentRoles S (c.map (rootCutInclusion G P S hSP)) := by
...
theorem root_reachable_stays_in_component {G : PartiteShape}
    {P S : Finset (Fin G.roles)} (c : G.C079CutComponent P)
    (hIso : RootComponentIsolatedAt c S)
...
theorem root_component_roles_map_eq (G : PartiteShape)
    (P S : Finset (Fin G.roles)) (hSP : S ⊆ P)
    (c : G.C079CutComponent P) (hIso : RootComponentIsolatedAt c S) :
    G.c079ComponentRoles S (c.map (rootCutInclusion G P S hSP)) =
  ...
  · intro hv
    ...
    have huNew := root_component_mem_map G P S hSP c u hu
    ...
    exact root_reachable_stays_in_component c hIso hu hReach
  · exact root_component_mem_map G P S hSP c v

theorem root_component_map_active (G : PartiteShape)
    (P S : Finset (Fin G.roles)) (hSP : S ⊆ P)
    (c : G.C079CutComponent P) (hActive : c.IsActive)
    (hIso : RootComponentIsolatedAt c S) :
    PartiteShape.C079CutComponent.IsActive (G := G) (cut := S)
      (c.map (rootCutInclusion G P S hSP)) := by
  have hRoles := root_component_roles_map_eq G P S hSP c hIso
  constructor
  · intro v hv
    rw [hRoles] at hv
...
theorem root_component_map_injective_on_isolated (G : PartiteShape)
    (P S : Finset (Fin G.roles)) (hSP : S ⊆ P)
    (c d : G.C079CutComponent P)
    (hc : RootComponentIsolatedAt c S) (hd : RootComponentIsolatedAt d S)
    (heq : c.map (rootCutInclusion G P S hSP) =
      d.map (rootCutInclusion G P S hSP)) : c = d := by
  have hRoles : G.c079ComponentRoles P c = G.c079ComponentRoles P d := by
    rw [← root_component_roles_map_eq G P S hSP c hc,
      ← root_component_roles_map_eq G P S hSP d hd, heq]
...
theorem root_isolated_active_component_card_le (G : PartiteShape)
    (P S : Finset (Fin G.roles)) (hSP : S ⊆ P) :
    Nat.card {c : G.C079CutComponent P // c.IsActive ∧ RootComponentIsolatedAt c S} ≤
      G.c079ActiveComponentCount S := by
  classical
  let A := {c : G.C079CutComponent P // c.IsActive ∧ RootComponentIsolatedAt c S}
  let Z := {c : G.C079CutComponent S // c.IsActive}
  letI : Fintype A := Fintype.ofFinite _
  letI : Fintype Z := Fintype.ofFinite _
  let f : A → Z := fun c => ⟨c.1.map (rootCutInclusion G P S hSP),
    root_component_map_active G P S hSP c.1 c.2.1 c.2.2⟩
  have hf : Function.Injective f := by
    intro c d h
    apply Subtype.ext
    exact root_component_map_injective_on_isolated G P S hSP c.1 d.1 c.2.2 d.2.2
...
def rootSeedState (seed : SeedFiber B forward) : ReplicaState G p :=
  Classical.choose (seedRecord_has_actual_state seed)

theorem rootSeedState_partition (seed : SeedFiber B forward) :
    (rootSeedState seed).partition = normalizedRecord (seedRecord seed) :=
  Classical.choose_spec (seedRecord_has_actual_state seed)

def rootSeedLayer (seed : SeedFiber B forward) (k : ℕ) : Finset (Fin G.roles) := by
  classical
  exact c079IntervalLayer
    (fun x => c079PartitionLower ((rootSeedState seed).partition x))
    (fun x => c079PartitionUpper ((rootSeedState seed).partition x)) k

theorem root_seedLayer_subset_backbone (seed : SeedFiber B forward) (k : ℕ) :
    rootSeedLayer seed k ⊆ family.backboneRoles := by
  classical
  intro x hx
  by_contra hxOff
  have hCount : partitionBlockCount ((rootSeedState seed).partition x) = p + 1 := by
    rw [rootSeedState_partition]
    exact normalizedRecord_off_blockCount seed x hxOff
  have hEnds : c079PartitionLower ((rootSeedState seed).partition x) =
      c079PartitionUpper ((rootSeedState seed).partition x) := by
    unfold c079PartitionLower c079PartitionUpper
    rw [hCount]
  have hMem : c079PartitionLower ((rootSeedState seed).partition x) < k ∧
      k ≤ c079PartitionUpper ((rootSeedState seed).partition x) :=
    (Finset.mem_filter.mp hx).2
  omega

theorem root_crossing_neighbors_mem_layer (seed : SeedFiber B forward)
    ...
    backboneNeighbors family c ⊆ rootSeedLayer seed k := by
  classical
  apply c079_refined_seed_neighbors_mem_intervalLayer (rootSeedState seed)
    (backboneNeighbors family c) (neighborMeet B forward c) k _ hCross
  intro x hx
  have hxBack := (Finset.mem_filter.mp hx).1
  rw [rootSeedState_partition, normalizedRecord_backbone seed x hxBack]
  change (backboneNeighbors family c).inf (repairedAt B forward) ≤ repairedAt B forward x
  exact Finset.inf_le hx

theorem root_crossing_component_isolated (seed : SeedFiber B forward)
    (c : G.C079CutComponent family.backboneRoles) (k : ℕ)
    (hCross : c079PartitionLower (neighborMeet B forward c) < k ∧
      k ≤ c079PartitionUpper (neighborMeet B forward c)) :
    RootComponentIsolatedAt c (rootSeedLayer seed k) := by
  classical
  intro v hv x hx e hev hex
  apply root_crossing_neighbors_mem_layer seed c k hCross
  exact Finset.mem_filter.mpr ⟨hx, v, hv, e, hex, hev⟩

abbrev RootCrossingFreeComponent
    (B : PathFiber p family d)
    (forward : ForwardCode (Fin G.roles) (p + 1) (3 * G.roles * offDefect family d))
    (k : ℕ) :=
...
theorem root_crossing_free_card_le_active (hCore : G.IsBoundaryCore)
    (seed : SeedFiber B forward) (k : ℕ) :
    Nat.card (RootCrossingFreeComponent B forward k) ≤
      G.c079ActiveComponentCount (rootSeedLayer seed k) := by
  classical
  let P := family.backboneRoles
  let S := rootSeedLayer seed k
  have hSP : S ⊆ P := root_seedLayer_subset_backbone seed k
  let A := RootCrossingFreeComponent B forward k
  let Z := {c : G.C079CutComponent S // c.IsActive}
  letI : Fintype A := Fintype.ofFinite _
  letI : Fintype Z := Fintype.ofFinite _
  have hActive (c : A) : c.1.1.IsActive :=
    ⟨c.1.2, G.c079_boundaryFree_component_attached_of_boundaryCore hCore P c.1.1 c.1.2⟩
  have hIso (c : A) : RootComponentIsolatedAt c.1.1 S :=
    root_crossing_component_isolated seed c.1.1 k c.2
  let f : A → Z := fun c => ⟨c.1.1.map (rootCutInclusion G P S hSP),
    root_component_map_active G P S hSP c.1.1 (hActive c) (hIso c)⟩
  have hf : Function.Injective f := by
    intro c d h
    apply Subtype.ext
    apply Subtype.ext
    exact root_component_map_injective_on_isolated G P S hSP c.1.1 d.1.1
      (hIso c) (hIso d) (congrArg Subtype.val h)
  have h := Fintype.card_le_of_injective f hf
  simpa only [A, Z, S, Nat.card_eq_fintype_card, Fintype.card_subtype,
    PartiteShape.c079ActiveComponentCount] using h

#print axioms root_component_roles_map_eq
#print axioms root_component_map_active
#print axioms root_component_map_injective_on_isolated
#print axioms root_isolated_active_component_card_le
#print axioms root_seedLayer_subset_backbone
#print axioms root_crossing_neighbors_mem_layer
#print axioms root_crossing_component_isolated
#print axioms root_crossing_free_card_le_active
