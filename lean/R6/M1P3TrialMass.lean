...
def rootP3Tilt (r : ℕ) (cS : ℝ) : ℝ := p3SyncEpsilon (96 * (r : ℝ) / cS)
def rootP3TrialCoefficient (r : ℕ) (pExt : ℝ) : ℝ := (min (pExt / 2) 1) ^ r

theorem root_P3Tilt_pos (r : ℕ) (cS : ℝ) (hr : 0 < r) (hcS : 0 < cS) :
    0 < rootP3Tilt r cS := by
  apply p3SyncEpsilon_pos
  positivity

theorem root_P3Tilt_exponent (r : ℕ) (cS : ℝ) (hr : 0 < r) (hcS : 0 < cS) :
    (r : ℝ) * (96 * (rootP3Tilt r cS) ^ 2 / cS) = (1 / 4 : ℝ) := by
...
theorem root_P3TrialCoefficient_pos (r : ℕ) (pExt : ℝ) (hpExt : 0 < pExt) :
    0 < rootP3TrialCoefficient r pExt := by
  unfold rootP3TrialCoefficient
  positivity

/-- A graph-fixed tilt keeps the total cost of at most r component tails at
most n^(1/4), including the empty product. -/
theorem root_P3_component_tail_product_lower
    (r k n : ℕ) (pExt cS : ℝ) (hr : 0 < r) (hk : k ≤ r) (hn : 1 ≤ n)
    (hpExt : 0 < pExt) (hcS : 0 < cS) :
    rootP3TrialCoefficient r pExt * (n : ℝ) ^ (-(1 / 4 : ℝ)) ≤
      (pExt * ((1 / 2 : ℝ) *
        Real.exp (-(96 * (rootP3Tilt r cS) ^ 2 * Real.log (n : ℝ) / cS)))) ^ k := by
  let a : ℝ := min (pExt / 2) 1
  let d : ℝ := 96 * (rootP3Tilt r cS) ^ 2 / cS
  ...
  have hbudget : (r : ℝ) * d = (1 / 4 : ℝ) := root_P3Tilt_exponent r cS hr hcS
  ...
  have hSingle : a * (n : ℝ) ^ (-d) ≤
      pExt * ((1 / 2 : ℝ) *
        Real.exp (-(96 * (rootP3Tilt r cS) ^ 2 * Real.log (n : ℝ) / cS))) := by
    have hEq : (n : ℝ) ^ (-d) =
        Real.exp (-(96 * (rootP3Tilt r cS) ^ 2 * Real.log (n : ℝ) / cS)) := by
...
theorem root_P3_trial_probability_bounds (r n : ℕ) (pExt : ℝ)
    (hpExt : 0 < pExt) (hn : 1 ≤ n) :
    0 ≤ rootP3TrialCoefficient r pExt * (n : ℝ) ^ (-(1 / 4 : ℝ)) ∧
      rootP3TrialCoefficient r pExt * (n : ℝ) ^ (-(1 / 4 : ℝ)) ≤ 1 := by
  have hc0 := (root_P3TrialCoefficient_pos r pExt hpExt).le
  have hc1 : rootP3TrialCoefficient r pExt ≤ 1 := by
    apply pow_le_one₀
    · positivity
...
theorem root_P3_floor_trial_mass_eventually (r : ℕ) (hr : 0 < r)
    (c : ℝ) (hc : 0 < c) :
    ∃ N : ℕ, ∀ n : ℕ, N ≤ n → 1 ≤ n ∧
  ...
  have hfloor := (root_uniformRoleDimension_bounds r n hr hn2).1
...
theorem root_P3_failure_factor_half (m : ℕ) (rho : ℝ)
...
#print axioms root_P3Tilt_exponent
#print axioms root_P3_component_tail_product_lower
#print axioms root_P3_trial_probability_bounds
#print axioms root_P3_floor_trial_mass_eventually
#print axioms root_P3_failure_factor_half
