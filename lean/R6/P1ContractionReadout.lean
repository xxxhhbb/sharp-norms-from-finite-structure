import R6.M1InternalSampleProduct
import R6.P3ActualP1ComponentLaw
import R6.C2AllSeparatorContraction
import R6.P1P2aEtaBridge
import R6.M1P3SampleReadoutBridge

/-! # Deterministic P2a/C2 contraction to P1 readout
...
/-- Retype a P2a interior assignment as the corresponding P1 assignment. -/
...
/-- Retype a P2a full component assignment as P1's identical component-role
label family. -/
...
/-- Retype a P2a boundary assignment using the occurrence-aware boundary
...
/-- The actual P2a internal monomial reads exactly P1's internal sample for
...
theorem p2a_componentInternalMonomial_eq_p1
    ...
    P2a.componentInternalMonomial dimension I K x =
      (p1InternalMonomialZ P dimension cut K.1
        ...
        (rootP1InternalSample P cut dimension I K) : ℝ) := by
  classical
  unfold P2a.componentInternalMonomial p1InternalMonomialZ
    rootP1InternalSample rootComponentInternalAddressMap
...
theorem p2a_componentGamma_eq_p1Gamma
    ...
    P2a.componentGamma dimension I K xB =
      p1Gamma P dimension cut K.1
        (p2aBoundaryAssignmentToP1 P dimension cut K xB)
        (rootP1InternalSample P cut dimension I K) := by
  classical
  ...
  calc
    ...
    _ = ∑ u : P1InteriorLabel P dimension cut K.1,
        (p1InternalMonomialZ P dimension cut K.1
          ...
          (rootP1InternalSample P cut dimension I K) : ℝ) :=
...
/-- After inserting the distinguished P1 boundary label, P2a's eta product
is the P1 second-layer character times the distinguished actual eta sign. -/
...
/-- The two Bool sign conventions used by P1 and `WeightedSignTilt` differ
...
theorem p2a_componentContraction_eq_neg_weightedSum_trial
  ...
  classical
  let d := p3DistinguishedBoundaryRole P cut K
  let epsI := rootP1InternalSample P cut dimension I K
...
/-- Absolute-value form consumed by the P3 tail event. -/
...
/-- Actual C2 contraction at a diagonal separator tuple, exposed directly in
the sample coordinates consumed by the P3 probability theorem. -/
theorem actualComponentContraction_abs_eq_p1WeightedSum
    (P : PaperShape) (dimension : Fin P.roles → ℕ)
    (cut : Finset (Fin P.roles))
    (omega : PaperR16.C2Actual.FrozenSample (G := P.toPartiteShape) cut dimension)
    (N : ℕ) (hN : ∀ u : P2a.SeparatorRole P.toPartiteShape cut,
      N ≤ dimension u.1)
    (i : Fin N) (K : P2a.ActiveComponent P.toPartiteShape cut) :
    |PaperR16.C2Actual.actualComponentContraction (G := P.toPartiteShape)
        cut dimension omega
      ...
      |WeightedSignTilt.weightedSum
        (fun j => p1Z P dimension cut K.1
          (p3DistinguishedBoundaryRole P cut K).1
          (p3DistinguishedBoundaryRole P cut K).2
          (rootP1InternalSample P cut dimension
            (PaperR16.C2Actual.frozenInternalCube (G := P.toPartiteShape)
              cut dimension omega) K)
          (p3ComponentSampleFromRest P dimension cut N hN i K
            (PaperR16.C2Actual.frozenRestCube (G := P.toPartiteShape)
              cut dimension omega)).1 j)
        (p3ComponentSampleFromRest P dimension cut N hN i K
          (PaperR16.C2Actual.frozenRestCube (G := P.toPartiteShape)
            cut dimension omega)).2| := by
  rw [PaperR16.C2Actual.actualComponentContraction_eq_p2a]
...
#print axioms p2aInteriorRoleEquivP1Interior
#print axioms p2a_componentInternalMonomial_eq_p1
#print axioms p2a_componentGamma_eq_p1Gamma
#print axioms p2a_componentEtaProduct_of_p1InsertBoundary
#print axioms p2a_componentContraction_eq_neg_weightedSum_trial
#print axioms p2a_componentContraction_abs_eq_weightedSum_trial
#print axioms p2a_componentContraction_abs_eq_weightedSum
#print axioms actualComponentContraction_abs_eq_p1WeightedSum
