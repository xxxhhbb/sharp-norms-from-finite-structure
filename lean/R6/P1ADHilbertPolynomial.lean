import R6.P1ADFiniteMean

/-! # Finite Hilbert sign-pair inequalities through order 32

The finite case split is intentional. The proof bodies use `ring` for every
polynomial identity and `norm_num` for the coefficient comparisons.
These proof bodies have NOT been executed in Lean in this delivery.
The Python generator supplies text only, never a logical certificate or oracle.
The single graph-independent constant is 32. No distributional hypothesis
or hypercontractivity theorem is imported as a premise.
-/
...
#print axioms sign_moment_coefficient_le
#print axioms euclidean_pair_moment_le
