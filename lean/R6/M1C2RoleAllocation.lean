...
open PaperR16 PaperR16.C2Actual PaperR16.ActualPrimitiveObservations
attribute [local instance] Classical.propDecidable

private theorem root_c2_subtype_heq {α : Type*} {P Q : α → Prop}
    (hpq : P = Q) (x : {w : α // P w}) (y : {w : α // Q w})
    (hval : x.1 = y.1) : HEq x y := by
  cases hpq
  exact heq_of_eq (Subtype.ext hval)

def rootActiveRoleSigmaEquiv (G : PartiteShape) (S : Finset (Fin G.roles)) :
...
theorem root_active_component_roles_sum (G : PartiteShape) (S : Finset (Fin G.roles)) :
  ...
  calc
    ...
    _ = Fintype.card ↥(activeUnion G S) := Fintype.card_congr (rootActiveRoleSigmaEquiv G S)
    _ = _ := Fintype.card_coe _

def rootC2SeparatorRoleEquiv (P : PaperShape) (S : Finset (Fin P.roles)) :
...
theorem root_C2_separator_card (P : PaperShape) (S : Finset (Fin P.roles)) :
    (C1C2.separator P S).card = S.card := by
  simpa only [Fintype.card_coe] using Fintype.card_congr (rootC2SeparatorRoleEquiv P S)

theorem root_C2_retained_card (P : PaperShape) (S : Finset (Fin P.roles)) :
...
theorem root_twoSide_card_allocation {R : Type*} [Fintype R] [DecidableEq R]
...
theorem root_C2_full_role_allocation (P : PaperShape) (S : Finset (Fin P.roles))
  ...
  have h := root_twoSide_card_allocation (C1C2.rows P S) (C1C2.cols P S)
    (C1C2.separator P S) hcover (C1C2.actual_inter P S hNoIso hMin)
  rw [root_C2_separator_card, root_C2_retained_card] at h
  rw [root_active_component_roles_sum]
  have hcard : (activeUnion P.toPartiteShape S).card ≤ P.roles := by
    simpa using Finset.card_le_univ (activeUnion P.toPartiteShape S)
  omega

#print axioms rootActiveRoleSigmaEquiv
#print axioms root_active_component_roles_sum
#print axioms root_C2_full_role_allocation
