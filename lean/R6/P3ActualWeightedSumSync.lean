import R6.P3ActualP1JointLaw
import R6.M1P3HalfInstanceAdapter
import R6.M1P3UniformTiltConstants
import R6.M1P3TrialMass
import R6.M1InternalGoodFloor

/-!
# P3: synchronized actual trials for all P1 weighted sums
-/
...
/-- The component-local P1 tail event used in one actual packed trial. -/
  ...
  eps * (n : ℝ) ^ ((P1AD.roleCount P cut K.1 : ℝ) / 2) *
    ...
    |weightedSum
      (fun j => p1Z P dimension cut K.1
        ...
        (rootP1InternalSample P cut dimension I K) x.1 j) x.2|

/-- A single actual separator tuple (one packed trial) succeeds for every active
component, with the P1 weighted sums read from the actual conditional cube. -/
...
has conditional synchronized P1 weighted-sum success probability at least one
...
theorem p3_actual_weightedSum_sync_half_uniform_floor
  ...
  obtain ⟨pExt, cS, CS, hpExt, hcS, hCS, htail⟩ :=
    root_p3_component_tail_uniform_tilt P a 1 ha hab
  let eps := rootP3Tilt P.roles cS
  have heps : 0 < eps := root_P3Tilt_pos P.roles cS hr hcS
  obtain ⟨nTail, hnTail, htail⟩ := htail eps heps
  let c := rootP3TrialCoefficient P.roles pExt
  have hc : 0 < c := root_P3TrialCoefficient_pos P.roles pExt hpExt
  obtain ⟨nMass, hMass⟩ :=
    root_P3_floor_trial_mass_eventually P.roles hr c hc
  ...
  have hEach : ∀ K : P2a.ActiveComponent P.toPartiteShape cut,
    ...
    have h := htail cut K.1 K.2
      ...
      (p3DistinguishedBoundaryRole P cut K).2
      dimension n hnTail'
      (root_uniformFloor_balanced P hr cut K.1 n hn2r)
      (rootP1InternalSample P cut dimension I K) (hI K)
    convert h using 1
  let k := Fintype.card (P2a.ActiveComponent P.toPartiteShape cut)
  have hk : k ≤ P.roles := by
    have hcard := root_activeComponent_card_eq_count P.toPartiteShape cut
    ...
    calc
      rho ≤ base ^ k := by
        simpa [rho, c, base, eps, k] using
          root_P3_component_tail_product_lower P.roles k n pExt cS
            hr hk hn1 hpExt hcS
  ...
  have hrhoBounds : 0 ≤ rho ∧ rho ≤ 1 := by
    simpa [rho, c] using root_P3_trial_probability_bounds P.roles n pExt hpExt hn1
  have hfailure : (1 - rho) ^ m ≤ (1 / 2 : ℝ) := by
    apply root_P3_failure_factor_half m rho hrhoBounds.1 hrhoBounds.2
    simpa [m, rho, c] using (hMass n hnMass').2
  have hsync := root_p3_some_trial_half_canonical
    P dimension cut m (fun _ => le_rfl) E rho hrhoBounds.1
      (by convert hrhoProduct using 1) hfailure
  convert hsync using 1

#print axioms p3WeightedSumComponentEvent
#print axioms p3WeightedSumSimultaneousEvent
